...
def valid(sess, model, mode='dev', record=True):

    print(f"Validating on {mode} set...")  # 添加打印语句，确认函数是否被调用
...
def train(sess, model):
    ...
    # Initialize best_dev_acc and best_iter
    best_dev_acc = 0.1
    best_iter = 0
        ...
        for doc, can, lab, loc in total_data:
            ...
            if cnt == FLAGS.batch_size:
                if not document or any(doc is None for doc in document):
                    print("Empty or invalid document batch, skipping")
                    document, candidates, ori_labels, ori_locs = [], [], [], []  # 清空当前 batch
                    cnt = 0  # 重置计数
                    continue
                ...
                if model.global_step.eval() % 100 == 0:
                    ...
                    if model.global_step.eval() % 1000 == 0:
                        ...
                        if dev_acc > best_dev_acc:
                            ...
                            best_iter = model.global_step.eval()
                            print("**  New best iteration  %d" % best_iter)
                        print("*   Best iteration  %d" % best_iter, flush=True)
    ...
    print(f"Skipped samples during training: {skipped_samples}")
    # 在训练结束时保存最终模型
    model.saver.save(sess, "%s/final_model" % train_dir)
    print("Model saved as final_model", flush=True)
...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        if FLAGS.is_train:
            ...
            valid(sess, model, "dev", record=True)  # 在开发集上输出结果

            # 在测试集上评估模型
            print("Evaluating on test set...")
            valid(sess, model, "test", record=True)  # 在测试集上输出结果
